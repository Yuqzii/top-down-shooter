...
TEST(Tree2D, General) {
  ...
  for (int i = 0; i < testPoints.size(); i++) {
    const vector2Df result =
        tree.findClosestObject(testPoints[i])->getPivotPosition();
    EXPECT_TRUE(result == expected[i])
        << "Expected: " << expected[i] << " Found: " << result;
  }
...
TEST(Tree2D, SinglePoint) {
  ...
  const vector2Df result =
      tree.findClosestObject(testPoints[1])->getPivotPosition();
...
TEST(Tree2D, DuplicatePoints) {
  ...
  objects.reserve(4);
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(10, 10))));
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(10, 10))));
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(15, 15))));
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(10, 10))));
  ...
  vector2Df result =
      tree.findClosestObject(vector2Df(5, 2))->getPivotPosition();
  EXPECT_TRUE(result == vector2Df(10, 10))
      << "Expected: " << vector2Df(10, 10) << " Found: " << result;

  result = tree.findClosestObject(vector2Df(1000000000, 1000000000))
               ->getPivotPosition();
  EXPECT_TRUE(result == vector2Df(10, 10))
      << "Expected: " << vector2Df(10, 10) << " Found: " << result;
...
TEST(Tree2D, DuplicateSinglePoints) {
  ...
  objects.reserve(4);
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(5, 3))));
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(5, 3))));
  objects.push_back(
      std::move(std::make_unique<MockGameObject>(vector2Df(5, 3))));
  ...
  EXPECT_THROW(tree.findClosestObject(vector2Df(5, 3)), int)
      << "Expected error 2 thrown.";

  vector2Df result =
      tree.findClosestObject(vector2Df(2, 12))->getPivotPosition();
...
TEST(Tree2D, MultiplePointQuery) {
  ...
  std::array<vector2Df, 3> testPoints = {vector2Df(2, 5), vector2Df(12, 12),
                                         vector2Df(25, 5)};

  std::array<std::vector<vector2Df>, 3> expected = {
      std::vector{vector2Df(3, 6), vector2Df(2, 7), vector2Df(9, 2)},
      std::vector{vector2Df(13, 15), vector2Df(17, 15), vector2Df(6, 12),
                  vector2Df(10, 19)},
      std::vector{vector2Df(17, 15), vector2Df(13, 15), vector2Df(9, 2),
                  vector2Df(6, 12), vector2Df(10, 19), vector2Df(3, 6)}};
...
TEST(Tree2D, ObjectsInRange) {
  ...
  std::array<vector2Df, 3> testPoints{vector2Df(12, 12), vector2Df(25, 5),
                                      vector2Df(2, 7)};
...
TEST(Tree2D, ObjectsInRange_OtherBranch) {
  ...
  std::vector<vector2Df> points = {vector2Df(9, 15), vector2Df(7, 10),
                                   vector2Df(16, 8), vector2Df(9, 9)};
  ...
  const std::vector<GameObject *> result =
      tree.findObjectsInRange(vector2Df(10, 10), 4);
