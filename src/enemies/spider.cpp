...
SpiderEnemy::SpiderEnemy(const float startHealth, const float damage,
                         const float moveSpeed, const float maxSteer,
                         const float steerMult, const float slowingRadius)
    : Enemy(startHealth, damage, moveSpeed, maxSteer, steerMult,
            slowingRadius) {
...
void SpiderEnemy::attackRangeCheck() {
  ...
  if (dist.crossProduct(dist) <= atkDist * atkDist) {
    ...
    const vector2Df playerDir(combatScene->player.getPivotPosition() -
                              pivotPosition);
...
void SpiderEnemy::avoidOtherEnemies(const float strength) {
  try {
    ...
    const Enemy *closest =
        combatScene->getEnemyManager().findClosestEnemy(pivotPosition);
    ...
    // Can't find closest enemy. Usually because there is currently only one
    // enemy. Does not require further action, hence why this catch is empty.
  }
}
