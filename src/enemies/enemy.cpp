...
Enemy::Enemy(const float health_, const float damage_, const float speed,
             const float steer, const float sMult, const float slowing)
    : startHealth(health_), damage(damage_), health(health_), moveSpeed(speed),
      maxSteer(steer), steerStrength(sMult), slowingRadius(slowing),
      isMoving(true),
...
void Enemy::initialize(const vector2Df &startPosition, const Scene &scene) {
  ...
  const vector2Df playerDirection =
      vector2Df(combatScene->player.getPivotPosition() - pivotPosition)
          .normalized();
...
void Enemy::update(Scene &scene, const float deltaTime) {
  if (isMoving) {
    ...
    rotation =
        velocity.toDegrees() + 90; // Rotate enemy in direction of movement
...
void Enemy::onCollision(const GameObject &other) {
  ...
  if (bullet == nullptr)
    return; // Return if colliding with something that is not a bullet
...
vector2Df Enemy::seek(const vector2Df &target) const {
  const vector2Df targetDirection(target -
                                  pivotPosition); // Find target direction
...
vector2Df Enemy::flee(const vector2Df &target) const {
  const vector2Df targetDirection(pivotPosition -
                                  target); // Find target direction
...
vector2Df Enemy::pursuit(const GameObject &target,
                         const float &predictionMultiplier) const {
  ...
  const vector2Df futurePosition =
      target.getPivotPosition() +
      target.getVelocity() * time * predictionMultiplier;
...
std::function<void(SDL_Renderer *)> Enemy::debugRender() const {
  return [this](SDL_Renderer *renderer) {
    GameObject::debugRender()(
        renderer); // Call parent debugRender and pass in renderer
