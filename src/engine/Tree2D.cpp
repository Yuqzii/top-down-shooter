...
void Tree2D::initializeWithList(const std::vector<GameObject *> &objects) {
  if (objects.empty())
    return; // Cannot build tree with no points
...
GameObject *Tree2D::findClosestObject(const vector2Df &target) const {
  ...
  return result
      ->object; // Return the GameObject associated with the result node
}

std::vector<GameObject *> Tree2D::findKClosestObjects(const vector2Df &target,
                                                      const int k) const {
  if (root == nullptr)
    throw 1;
...
std::vector<GameObject *> Tree2D::findObjectsInRange(const vector2Df &target,
  ...
  auto nodesRange =
      nodes | std::views::transform([](const Node *node) -> GameObject * {
        return node->object;
      });
...
void Tree2D::initializeTree(const std::vector<GameObject *> &objects) {
  ...
  for (GameObject *object : objects) {
    const std::array<float, 2> point = {object->getPivotPosition().x,
                                        object->getPivotPosition().y};
    sorted.push_back(
        std::make_pair(std::array<float, 2>{object->getPivotPosition().x,
                                            object->getPivotPosition().y},
                       object));
  }
  ...
  for (int i = 0; i < sorted.size(); i++) {
    if (i == sorted.size() / 2)
      continue; // Do not insert median again
    insertRecursive(root, sorted[i].first, sorted[i].second, 0); // Insert point
  }
}

Tree2D::Node *Tree2D::insertRecursive(Node *node,
...
const Tree2D::Node *Tree2D::nearestNeighbor(const Node *node,
  ...
  const int dimension =
      depth % 2; // Calculate the current dimension of the tree
  ...
  // If distance to split is smaller than to the closest node there is a
  // possibility that there exists a closer node in that branch of the tree
...
const Tree2D::Node *Tree2D::kNearestNeighbors(
  ...
  if ((dist * dist <= radiusSquared || heap.size() < k) &&
      otherBranch != nullptr) {
...
void Tree2D::nodesInRange(const Node *node, const std::array<float, 2> &target,
                          const int depth, const float range,
                          std::vector<const Node *> &nodesList) const {
  ...
  // If distance to split is smaller than the search range there is a
  // possibility that there exists a valid node in that branch of the tree
...
void Tree2D::updateHeap(std::list<std::pair<float, const Node *>> &heap,
                        const Node *node, const std::array<float, 2> &target,
                        const int k) const {
