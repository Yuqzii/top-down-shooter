...
GameObject::GameObject() : boundingCircle(500.0f), useCollision(false) {
  ...
  // on)
  destRect.w =
      srcRect.w * 3; // Create global macro or sum for size instead of 3?
...
void GameObject::initialize(const vector2Df &startPosition,
                            const Scene &scene) {
  // Load texture
...
void GameObject::checkCollisions(const Scene &scene) {
  ...
  try {
    ...
    std::cerr << "Exception " << e
              << " when checking collisions. Tree was likely not built.\n";
  }
  ...
  for (GameObject *object : closeObjects) {
    ...
    case POINT:
      if (Collision::checkCollision(object->getPivotPosition(),
                                    circleCollider)) {
        addCollision(object);
      }
      break;
...
void GameObject::animationUpdate(Scene &scene, const double &deltaTime) {
  ...
  if (animationCounter >= sequence.length) {
    animationCounter -=
        sequence.length; // -= length so that the animation plays at same speed
  }
...
void GameObject::changeAnimation(const int sequenceId) {
  ...
  if (animationSequence == sequenceId)
    return;
