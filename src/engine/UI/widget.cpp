...
void Widget::calculatePosition(const bool &calculateChildren) {
  ...
  if (parent == nullptr) {
    ...
    // Calculate parents position without updating its children to avoid
    // infinite recursion
    ...
    // Update position
    position.x =
        parent->position.x + parent->size.x * (localPosition.x + add.x) / 100;
    position.y =
        parent->position.y + parent->size.y * (localPosition.y + add.y) / 100;
  }
