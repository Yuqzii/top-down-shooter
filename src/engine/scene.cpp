...
template <class T> T &Scene::instantiate(const vector2Df &position) {
  ...
  // Create the new GameObject as a unique_ptr to clarify that Scene has
  // ownership
...
template SpiderEnemy &
Scene::instantiate<SpiderEnemy>(const vector2Df &position);
template EnemyAttackPoint &
Scene::instantiate<EnemyAttackPoint>(const vector2Df &position);
...
void Scene::updateObjectTree() {
  ...
  const auto objectsRange =
      gameObjects |
      std::views::transform(
          [](const std::unique_ptr<GameObject> &ptr) -> GameObject * {
            return ptr.get();
          });
  const std::vector<GameObject *> rawObjects(objectsRange.begin(),
                                             objectsRange.end());
