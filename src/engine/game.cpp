...
void Game::update() {
  ...
  const float deltaTime =
      (float)(nowTime - prevTime) / (float)SDL_GetPerformanceFrequency();
  prevTime = nowTime;
...
template <class T> void Game::addScene() {
  // Compile time check that added scenes are in fact scenes
  static_assert(std::is_base_of<Scene, T>(),
                "Scene must derive from type Scene.");
