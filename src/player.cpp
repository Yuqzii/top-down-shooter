...
Player::Player()
    : healthbarBG(vector2Df(20, 0), vector2Df(250, 30),
...
void Player::update(Scene &scene, const float deltaTime) {
  ...
  moveDir = moveDir.normalized(); // Normalize vector so that diagonal movement
                                  // is not faster
  velocity = moveDir * moveSpeed; // Update velocity

  GameObject::update(
      scene, deltaTime); // Call base GameObject update (Updates position)
  ...
  // Make space from bottom of screen to healthbar same as side of screen to
  // healthbar
...
inline void Player::shoot(Scene &scene) const {
  ...
  // Instantiate bullet
  constexpr float distMultiplier =
      50; // How much further than player center should bullet spawn
...
void Player::onCollision(const GameObject &other) {
  ...
  if (enemyCollisionPoint ==
      nullptr) // Return if collision is not with enemy attack
    return;
