...
void Bullet::checkCollisions(const Scene &scene) {
  ...
  for (GameObject *object : closeObjects) {
    if (!object->getUseCollision() || collisionList.count(object) ||
        object == this)
      continue;
    ...
    if (Collision::checkCollision(object->circleCollider, movementLine)) {
      ...
      break; // Bullet is deleted upon collision anyways, no point in checking
             // more
    }
  }
}
...
void Bullet::onCollision(const GameObject &other) {
  // Delete object when colliding with something and stop further collision
  // updates
  deleteObject = true;
  throw 1;
}

void Bullet::initializeDirection(const vector2Df direction,
