...
public:
  ...
  // Returns the closest object to target in the tree, that is not the same as
  // target
  GameObject *findClosestObject(const vector2Df &target) const;

  // Returns an std::vector of the k closest objects that are not the same as
  // target
  std::vector<GameObject *> findKClosestObjects(const vector2Df &target,
                                                const int k) const;
  ...
  std::vector<GameObject *> findObjectsInRange(const vector2Df &target,
...
private:
  ...
  const Node *kNearestNeighbors(const Node *node,
                                const std::array<float, 2> &target,
                                const int depth,
                                std::list<std::pair<float, const Node *>> &heap,
                                const int k) const;
  void updateHeap(std::list<std::pair<float, const Node *>> &heap,
                  const Node *node, const std::array<float, 2> &target,
                  const int k) const;

  void nodesInRange(const Node *node, const std::array<float, 2> &target,
